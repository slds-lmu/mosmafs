...
getStatistics <- function(log) {
  ...
  if ("runtime.mean" %in% colnames(stats)) {
    ...
    stats$runtime.mean <- rnorm(nrow(stats), 0.034, 0.002) * (fid + 0.2)
...
It is possible to inspect the runtimes of individuals, they are stored as attribute `"runtime"`. The aggregated runtime for each generation can also be retrieved from the `log.newinds` logging object. 
