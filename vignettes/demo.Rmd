...
print.list <- function(x) {
  if (all(vlapply(x, is.atomic))) {
    catf("list(%s)", collapse(sprintf("%s=%s", names(x), vcapply(x, deparse)), ", "))
  } else {
    NextMethod(x)
  }
}
...
`mosmafs` crucially depends on the `ecr` package (currently the github-version!), so make sure you have it installed and loaded.
...
One can use the `sampleValue` function from the `ParamSet` package to generate valid, randomly sampled individuals to use with combined operators. The `discrete.names` option must be set to `TRUE` for that:
...
The initial population to sample over can be generated using the `ParamHelpers` function `sampleValues()`:

```{r}
initials.simple <- sampleValues(ps.simple, 15, discrete.names = TRUE)
```

This is all that is necessary, and it can be used to call the `ecr()` function in the `ecr` package. The `mosmafs` package comes with a useful interface that sets some defaults: `mosmafs.nsga2()`.
...
```{r}
crossover.simple <- combine.operators(ps.strat,
  numeric = recPCrossover,
  integer = recPCrossover,
  selector.selection = recPCrossover)
```
...
```{r}
initials.strat <- sampleValues(ps.simple, 15, discrete.names = TRUE) %>%
  lapply(initials.strat, function(x) {
    x$selector.selection %<>% as.numeric %>%
      mutUniformMetaReset(p = 1, fima, x$filterweights) %>%
      as.logical
    x
  })
```
